...
def launch_independent_async_handling_process(data):
    ...
    if settings.config['TIMESHEET_DEBUG_MODE']:
        sys_descr_stdout, _ = tempfile.mkstemp(suffix = '.debug', prefix = 'exec_log_stdout_' + ("%.2f"%(time.time())))
        sys_descr_stderr, _ = tempfile.mkstemp(suffix = '.debug', prefix = 'exec_log_stderr_' + ("%.2f"%(time.time())))
        subprocess.Popen([sys.executable, os.path.join(settings.BASE_DIR, 'manage.py'), 'analyze_slack_request', '--request_file', file_path], stdout=sys_descr_stdout, stderr=sys_descr_stderr)
    else:
        subprocess.Popen([sys.executable, os.path.join(settings.BASE_DIR, 'manage.py'), 'analyze_slack_request', '--request_file', file_path], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
...
def handle_slack(request):
    ...
    if decoded_body.startswith('payload='):
        try:
            ...
            if time.time() > float(settings.config['SLACK_QUERY_MAX_AGE_SECONDS']) + float(request_timestamp):
                raise ValueError('Query is too old hence suspect')
            data_signature = 'v0=' + hmac.new(settings.config['SLACK_SIGNING_SECRET'].encode('utf-8'), ('v0:' + request_timestamp + ':').encode('utf-8') + request.body, hashlib.sha256).hexdigest()
            ...
            if 'type' in data_as_dict:
                if ((data_as_dict['type'] == 'view_submission') or (data_as_dict['type'] == 'block_actions')):
