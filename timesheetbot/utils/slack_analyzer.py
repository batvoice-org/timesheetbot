...
def parse_modal_date(date_as_text):
    ...
    date_splitted = [int(part) for part in date_as_text.split(',')[0].strip().split(' ')[1].strip().split('-')]

    return {'date':datetime.date(date_splitted[0],date_splitted[1],date_splitted[2]), 'is_morning':(day_period == 'morning'), 'is_afternoon':(day_period == 'afternoon')}
...
class SlackAnalyzer:
    ...
    def analyze_and_respond(self):
        ...
        self.user_analyzer = UserAnalyzer(User.objects.filter(slack_userid=self.request_data['user']['id']).values('pk').get()['pk'])
    ...
    def handle_data_modification(self, action_type='submit'):
        ...
        data_concerned_date = parse_modal_date(self.request_data['view']['blocks'][0]['label']['text'])
